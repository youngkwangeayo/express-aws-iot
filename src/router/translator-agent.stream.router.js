...
translatorAgentStreamRouter.post('/menu-translation/:lang', validateMenuTranslation, async (req, res,) => {
    ...
    streamChunk.resWrite("언어 번역을 시작합니다.");
    try {
        await translatorAgentService.runTranslateMenuProcess(req.matchedData.frId, req.matchedData.lang, req.matchedData.overwrite, cookie, authHeader, streamChunk.resWrite);
    } catch (error) {
        debug(error);
        return streamChunk.resError(error.message);
    };
