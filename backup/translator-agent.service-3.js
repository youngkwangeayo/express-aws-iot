...
async function* runTranslateMenuProcess(frId, lang, cookie, authHeader) {
    ...
    const menuGen = translateMenu(frId, lang, cookie, authHeader);
    const categoryGen = translateCategory(frId, lang, cookie, authHeader);
    const productGen = translateProduct(frId, lang, cookie, authHeader);
...
async function* translateProduct(frId, lang, cookie, authHeader) {
    ...
    // Promise를 시작하지만 await하지 않음
    const startTime = Date.now();
    const translationPromise = cmsAgent.translateJSON(taskResult_product, lang);
...
async function* runTranslateMenuProc2ess2(frId, lang, cookie, authHeader) {
...
async function* runTranslateMe123123nuProcess(frId, lang, cookie, authHeader) {
    const streams = [translateMenu(frId, lang, cookie, authHeader), translateCategory(frId, lang, cookie, authHeader), translateProduct(frId, lang, cookie, authHeader)];
    const readers = streams.map((s) => s[Symbol.asyncIterator]());

    // "각 스트림에서 next() 실행 결과"를 프로미스로 묶는다
    const nextPromises = readers.map((reader, index) =>
        reader.next().then(result => ({ index, result }))
    );

    while (nextPromises.length > 0) {
        // 가장 먼저 완료되는 next() 결과를 가져온다
        const { index, result } = await Promise.race(nextPromises);

        if (result.done) {
            // 해당 reader는 끝났으니 제거
            const idx = nextPromises.findIndex(p => p.index === index);
            ...
            // 다음 chunk 요청을 다시 등록
            nextPromises[index] = readers[index]
                .next()
...
async function* runTranslate123123MenuProcess(frId, lang, cookie, authHeader) {

    // translateMenu(frId, lang, cookie, authHeader)
    // translateCategory(frId, lang, cookie, authHeader)
    // translateProduct(frId, lang, cookie, authHeader)
